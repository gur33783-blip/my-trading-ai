...
with st.sidebar:
    ...
    if user_query:
        responses = ["Nifty support le raha hai bhai.", "Momentum bullish lag raha hai.", "Dhyan se, VIX badh raha hai!"]
...
if df is not None:
    ...
    fig.update_layout(height=500, template="plotly_dark", paper_bgcolor="#0d0e12", plot_bgcolor="#0d0e12", xaxis_rangeslider_visible=False)
    st.plotly_chart(fig, use_container_width=True)

# --- CHULBULI TIPS ---
if 'last_tip' not in st.session_state or time.time() - st.session_state.get('last_time', 0) > 180:
    tips = ["Bhai, profit ho toh screen off!", "Guri PB13 signal hai, toh darne ki kya baat?", "Market nakhre kare toh wait karo!"]
    st.session_state.last_tip = random.choice(tips)
    st.session_state.last_time = time.time()

# YEH HAI FIX: Triple quotes properly closed
st.markdown(f"""
    <div style="background: rgba(255,255,255,0.05); padding:15px; border-radius:10px; border-left:4px solid {theme_color};">
        💡 <b>Guri's Tip:</b> {st.session_state.last_tip}
    </div>
""", unsafe_allow_html=True)
