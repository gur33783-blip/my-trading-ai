...
# --- 1. UI & GROWW THEME ---
st.set_page_config(page_title="GURI GHOST V7.4", layout="wide")
...
# --- 2. MULTI-DATA ENGINE (STOCKS + SECTORS + GLOBAL) ---
@st.cache_data(ttl=1)
def fetch_everything(idx, interval):
    targets = {
...
with st.sidebar:
    ...
    st.divider()
    st.info("Bhai, yahan saare indicators live hain. Kuch bhi hide nahi hai.")

# --- 4. MAIN TERMINAL ---
tf = st.select_slider("⚡ TIMEFRAME", options=["1m", "5m", "15m", "30m", "1h"], value="5m")

@st.fragment(run_every=1)
def final_terminal_v74(idx_name, timeframe):
    data = fetch_everything(idx_name, timeframe)
    if 'df' in data:
        df = data['df']
        ...
        st.markdown(f"""
            <div style="display:flex; gap:10px; margin-bottom:15px;">
                ...
                <div style="background:#111; padding:5px 12px; border-radius:8px; border:1px solid #333;">🚀 SIGNAL: {'BULLISH' if data['NASDAQ']['change']>0 else 'BEARISH'}</div>
            </div>
        """, unsafe_allow_html=True)
        ...
        with col_pulse:
            ...
            if last_macd > last_sig:
                st.markdown("""<div class="sniper-alert" style="color:#00d09c; border-color:#00d09c; background:rgba(0,208,156,0.1);">🐂 CALL ENTRY</div>""", unsafe_allow_html=True)
            else:
                st.markdown("""<div class="sniper-alert" style="color:#eb5b5d; border-color:#eb5b5d; background:rgba(235,91,93,0.1);">🐻 PUT ENTRY</div>""", unsafe_allow_html=True)

        with col_chart:
            # MAIN PRICE
            st.markdown(f"""<div style="font-size:40px; font-weight:800;">₹{data['MAIN']['price']:,.2f} <span style="font-size:18px; color:{'#00d09c' if data['MAIN']['change']>0 else '#eb5b5d'};">{data['MAIN']['change']:+.2f}%</span></div>""", unsafe_allow_html=True)
            
            # CHART WITH ARROWS
            fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.03, row_heights=[0.8, 0.2])
            fig.add_trace(go.Candlestick(x=df.index, open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'], increasing_line_color='#00d09c', decreasing_line_color='#eb5b5d', name='Price'), row=1, col=1)
            ...
            fig.update_layout(height=600, template="plotly_dark", xaxis_rangeslider_visible=False, margin=dict(l=0,r=0,t=0,b=0), paper_bgcolor='black', plot_bgcolor='black')
            st.plotly_chart(fig, use_container_width=True)

target = st.selectbox("MARKET", ["NIFTY", "BANKNIFTY"], label_visibility="collapsed")
final_terminal_v74(target, tf)
