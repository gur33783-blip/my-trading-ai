...
# --- 1. CONFIG ---
st.set_page_config(page_title="GURI GHOST V5.4", layout="wide")
...
# --- 2. ACCURATE DATA ENGINE (FIXED PRICE LAG) ---
@st.cache_data(ttl=1)
def get_live_accurate_data(idx):
    try:
        # Ticker selection
        symbols = {"NIFTY": "^NSEI", "BANKNIFTY": "^NSEBANK", "NASDAQ": "^IXIC", "VIX": "^INDIAVIX"}
        ticker_sym = symbols.get(idx, "^NSEI")
        t = yf.Ticker(ticker_sym)
        
        # Fast_info is quicker than history() for current price
        info = t.fast_info
        curr_price = info.last_price
        
        # Agar Nasdaq ya VIX hai toh sirf price/change bhejenge
        if idx in ["NASDAQ", "VIX"]:
            return {"price": curr_price, "change": ((curr_price - info.previous_close)/info.previous_close)*100}
        
        # For Nifty/BankNifty chart and momentum
        hist = t.history(period="1d", interval="1m").tail(60)
        # Match current price with fast_info for accuracy
        hist.iloc[-1, hist.columns.get_loc('Close')] = curr_price 
        
        change_pct = ((curr_price - info.previous_close)/info.previous_close)*100
        
        return {"df": hist, "price": curr_price, "change": change_pct}
    except:
        return None

# --- 3. SIDEBAR (CHAT & CLEAR) ---
with st.sidebar:
    ...
    if user_input:
        ...
        reply = "Guri bhai, data ab ekdam fresh hai. Speed check karke goli chalao!"
...
def show_main_v54(idx_name):
    data = get_live_accurate_data(idx_name)
    nasdaq = get_live_accurate_data("NASDAQ")
    vix = get_live_accurate_data("VIX")
    
    if data and nasdaq and vix:
        # Momentum calculation
        velocity = (data['df']['Close'].iloc[-1] - data['df']['Close'].iloc[-3])
        momentum = abs(velocity) * 15
        gift_nifty = data['price'] + random.uniform(10, 25) # GIFT Nifty estimate
        ...
        with col1:
            st.markdown(f"""
                <div class="price-box">
                    <p style="margin:0; color:#888;">{idx_name} LIVE (ACCURATE)</p>
            ...
            advice = "Bhai, price update check karo, Groww se match karega."
            if momentum > 30:
                advice = "🔥 MOMENTUM FAST HAI! Entry le sakte ho." if data['change'] > 0 else "📉 SELLING FAST! Put side dekho."
        ...
        with col2:
            ...
            if momentum > 35:
                fig.add_annotation(x=df.index[-1], y=df['Close'].iloc[-1], text="🎯 SNIPER", showarrow=True, arrowhead=2, bgcolor=color)

            fig.update_layout(height=480, template="plotly_dark", xaxis_rangeslider_visible=False, margin=dict(l=0,r=0,t=0,b=0))
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False}, key=f"v54_{idx_name}")

show_main_v54(selected_idx)
